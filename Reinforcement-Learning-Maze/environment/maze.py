...
class Maze:
    ...
    reward_exit = 16.0  # reward for reaching the exit cell
    ...
    def __init__(self, maze, start_cell=(0, 0), exit_cell=None):
        ...
        self.__minimum_reward = -2.0 * self.maze.size  # stop game if accumulated reward is below this threshold
    ...
    def reset(self, start_cell=(0, 0)):
        ...
        self.__visited = [] # empty list so that track of number of visits can be done
    ...
    def __execute(self, action):
        ...
        if not possible_actions:
            ...
            if self.__current_cell == self.__exit_cell:
                ...
                no_of_visits = self.__visited.count(self.__current_cell) # to calculate the number of times the cell visited
                # reward = (Maze.penalty_visited)*(no_of_visits) # penalty1 : Linear scaling of penalty
                reward = (Maze.penalty_visited)*(2**(no_of_visits)) # Penalty 2 : powered penalty for action for revisit of the cell
                #reward = Maze.penalty_visited  # Original(Constant): Constant penalty when returning to a cell which was visited earlier
            else:
                reward = Maze.penalty_move  # penalty for a move which did not result in finding the exit cell

            self.__visited.append(self.__current_cell)
        else:
